...
bot = Client("bot",
             bot_token= "7303931675:AAE6f1i2WKrq8eB70cEuJ5rAetCP4tybC6Y",
             api_id= 29640188,
             api_hash= "e470abc84a3bc445997ee4ea5be87deb")
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1001833358236, x)
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:                               
                cc = f'** {str(count).zfill(3)}.** {name1} ({res}) 𝚂𝚝𝚞𝚋𝚋𝚘𝚛𝚗.mkv\n**Batch Name :** {b_name}\n\n**Downloaded by : {CR}**'
                cc1 = f'** {str(count).zfill(3)}.** {name1} 𝚂𝚝𝚞𝚋𝚋𝚘𝚛𝚗.pdf \n**Batch Name :**{b_name}\n\n**Downloaded by : {CR}**'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1001833358236)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
